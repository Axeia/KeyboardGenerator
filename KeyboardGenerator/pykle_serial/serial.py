...
class Key:
    ...
    textColor: List[Optional[str]] = dcf(
        default_factory=_default_factory_list_factory(UB_LABEL_MAP))
    textSize: List[Optional[int]] = dcf(
        default_factory=_default_factory_list_factory(UB_LABEL_MAP))
...
def deserialize(rows: List) -> Keyboard:  # noqa: C901
    ...
    for r, rows_r in enumerate(rows):
        if isinstance(rows_r, list):
            ...
            if r != 0:
                _deserialize_error(
                    "keyboard metadata must the be first element", rows_r)
